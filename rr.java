...
class myprocess {
    ...
    int id[] = new int[100]; //processid
    int at[] = new int[100]; //arrival time
    int bt[] = new int[100]; //burst-time
    int st[] = new int[100]; //starting-time
    int ct[] = new int[100]; //completion-time
    int tat[] = new int[100]; //turnaround
    int wt[] = new int[100]; //waiting
    int rt[] = new int[100]; //response time
    int bt_rem[] = new int[100]; //burst time remaining
}
...
public class rr {
    ...
    public static void main(String[] args) {
        ...
        avgtrt = (float) trt / num;

        System.out.println("ID\t" + "AT\t" + "BT\t" + "ST\t" + "CT\t" + "TAT\t" + "WT\t" + "RT");

        for (int i = 0; i < num; i++) {
            System.out.println("P" + p.id[i] + "\t" + p.at[i] + "\t" + p.bt[i] + "\t" + p.st[i] + "\t" + p.ct[i] + "\t"
                    + p.tat[i] + "\t" + p.wt[i] + "\t" + p.rt[i] + "\n");
        }

        System.out.println("Average TurnAround Time: " + avgtat);
        System.out.println("Average Waiting Time: " + avgtwt);
        System.out.println("Average Response Time: " + avgtrt);
